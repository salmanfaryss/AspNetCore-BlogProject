...
namespace CoreDemo.Controllers
{
    ...
    public class MessageController : Controller
    {
        ...
        public IActionResult SendBoxMessage()
        {
            var username = User.Identity.Name;
            var usermail = c.Users.Where(x => x.UserName == username).Select(y => y.NameSurname).FirstOrDefault();
            var writerID = c.writers.Where(x => x.WriterName == usermail).Select(y => y.WriterID).FirstOrDefault();
        ...
        [HttpGet]
        public async Task<IActionResult> SendNewMessage()
        {
        ...
        [HttpPost]
        public IActionResult SendNewMessage(Message2 p)
        {
            var username = User.Identity.Name;
            var usermail = c.Users.Where(x => x.UserName == username).Select(y => y.NameSurname).FirstOrDefault();
            var writerID = c.writers.Where(x => x.WriterName == usermail).Select(y => y.WriterID).FirstOrDefault();
            ...
            mm.TInsert(p);
            return RedirectToAction("InboxMessage","Message");
        }
        [HttpGet]
